...
function Main({navigation}) {
  ...
  const { cartProducts, getProducts, setCurrentShopping, toggleOverlay, setStorage } =
    useCartProduct();
  ...
  const toggleOverlayConfirmation = () => {
    setVisible(!visible);
  };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const {data} = response
      setStorage("currentShopping", JSON.stringify(data.shopping))
      setCurrentShopping(data.shopping);
      toggleOverlayConfirmation();
      toggleOverlay();
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      {products && products.length > 0 ? (
        <View>
          ...
          />
          <View
            style={{ position: "relative", alignItems: "center", margin: 5 }}
          >
            <Button
              icon={
                <Icon
              ...
              iconRight
              onPress={() => toggleOverlayConfirmation()}
          ...
          <Overlay
            ...
            isVisible={visible}
            onBackdropPress={toggleOverlayConfirmation}
          >
            <Card>
              ...
              <View style={{ alignItems: "center", marginHorizontal: 15 }}>
                ...
                >
                  Valor frete: R$: {shippingValue}{" "}
                </Text>
                <Text
                  h5
                  style={{ color: "#4f9deb", fontSize: 15, marginBottom: 10 }}
                >
                  Valor do pedido: R$: {productsSum}{" "}
                </Text>
                <Text
                  h5
                  style={{ color: "#4f9deb", fontSize: 15, marginBottom: 10 }}
                >
                  Valor total do pedido: R$: {totalValue}{" "}
                </Text>
              </View>
              <Button
                icon={
