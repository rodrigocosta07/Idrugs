...
function Main() {
  ...
  const getStatus = async () => {
    try {
      ...
      const { data } = response;
      const currentStatus = handleStatus(data.shopping.status);
      ...
      {currentShopping.id ? (
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <Card style={{ width: "100%" }}>
            ...
            <ProgressBar
              progress={status === "Pedido entregue" ? 100: 0.5}
              indeterminate={status === "Pedido entregue" ? false: true}
...
export default function ShoppingStatus({ route, navigation }) {
  return (
    <>
